...
void Dice::Load() {
  ...
  enableShakeForDice = !System::SystemTask::displayApp->settingsController.isWakeUpModeOn(Controllers::Settings::WakeUpMode::Shake);
...
void Dice::Refresh() {
  ...
  if (System::SystemTask::displayApp->motionController.CurrentShakeSpeed() >=
      System::SystemTask::displayApp->settingsController.GetShakeThreshold()) {
...
void Dice::roll() {
  ...
  if (openingRoll == false) {
    System::SystemTask::displayApp->motorController.RunForDuration(30);
    NextColor();
    currentRollHysteresis = rollHysteresis;
  }
}

void Dice::btnRollEventHandler(lv_obj_t* obj, lv_event_t event) {
  if (event == LV_EVENT_CLICKED)
    static_cast<Dice*>(obj->user_data)->roll();
}
