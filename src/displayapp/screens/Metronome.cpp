...
void Metronome::Refresh() {
  if (metronomeStarted) {
    if (xTaskGetTickCount() - startTime > 60u * configTICK_RATE_HZ / static_cast<uint16_t>(bpm)) {
      ...
      if (counter == 0) {
        counter = bpb;
        System::SystemTask::displayApp->motorController.RunForDuration(90);
      } else {
...
void Metronome::eventHandler(lv_obj_t* obj, lv_event_t event) {
  static_cast<Metronome*>(obj->user_data)->onEvent(obj, event);
}

lv_obj_t* Metronome::createLabel(const char* name, lv_obj_t* reference, lv_align_t align, lv_font_t* font, uint8_t x, uint8_t y) {
